...
## Phase 8 — Version & publish to npm ✅ (PUBLISHED)

Coordinated open-core release, **research-grounded** in the [changesets](https://github.com/changesets/changesets)
workflow (release intent as changelog records; `access: public` for scoped packages).

- **Versioned all 46 surviving `@weaveintel/*` packages to `0.1.1`** (uniform — 2 were already at 0.1.0 on
  npm, so a plain 0.1.0 would have collided); repointed 145 internal dep refs to `^0.1.1`. Apps
  (geneweave-api/ui@1.0.0, live-agents-demo@0.1.0) excluded + kept at their versions. CHANGELOG.md +
  `.changeset/open-core-restructure.md` record. Command scaffolding in `scripts/phase8/`.
- **PUBLISHED all 46 packages at 0.1.1** to the public registry (`scripts/phase8/publish.sh`,
  foundational-first). Verified live: core/tools/collab/weaveintel/resilience/identity/guardrails all report
  0.1.1.
- **DEPRECATED all 44 retired names** (`scripts/phase8/deprecate.sh`), each with a message naming its
  replacement (e.g. `@weaveintel/coedit` → `@weaveintel/collab`, `@weaveintel/tools-gmail` →
  `@weaveintel/tools/gmail`). Nothing unpublished — existing installs keep working while adopters migrate.
- **GATE MET — smoke test** (`scripts/phase8/smoke-test.sh`): in a scratch dir OUTSIDE the repo, installed
  `weaveintel` + `collab` + `tools` @0.1.1 from the registry and a 20-line app both **typechecks and runs**
  against registry-only packages — including the tree-shakeable subpath `@weaveintel/tools/gmail`.

**Notes / deviations:** `a11y` is NOT retired (kept standalone in Phase 4 vs the plan's assumption), so 46
published / 44 deprecated (not 45). The unscoped `weaveintel` alias (plan: "attempt it") was NOT published —
optional, needs a separate package; deferred. The deprecate messages link to `MIGRATION.md` on `main`, which
resolves once this branch merges.

**Package count: 87 → 46 published.** Remaining: Phase 9 (private-repo sync), Phase 10 (final report + tag).
