...
- **Remaining (same recipe, mechanical):** the other 9 workflow stores (definition/run-repo/idempotency/payload/sleep/step-lock/rate-limiter/run-queue/audit-log) and the memory/live-agents/triggers SQL follow the identical schema-+-shared-impl-+-contract recipe proven here. Not done in this pass to keep the change safe and reviewable; the checkpoint store is the template.
