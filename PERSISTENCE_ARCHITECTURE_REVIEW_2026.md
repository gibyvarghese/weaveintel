...
- **triggers + memory collapsed (follow-on pass).** `@weaveintel/triggers` (`weavePostgresTriggerStore` / `weaveSqliteTriggerStore`, two tables + the W6-fields-in-metadata packing) and `@weaveintel/memory` (`weavePostgresMemoryStore` / `weaveSqliteMemoryStore`, the plain `memory_entries` JSON store; pgvector stays Postgres-only) now each share ONE Drizzle query implementation behind their existing ports — same public factory APIs, on-disk shapes unchanged (backward compatible). Drizzle's `boolean()` vs `integer({mode:'boolean'})` cleanly handles triggers' `enabled`; memory's `clear(filter)` moved from dialect-specific `json_extract`/`->>` SQL to one shared JS matcher (identical rows deleted). New shared contracts (`triggerStoreContract`, `memoryStoreContract`) run against the in-memory reference, SQLite, and real Postgres. **Real Postgres coverage that never ran in-package before**, plus stress (2,000 triggers / 5,000-row invocation ledger; 5,000 memories), injection safety, and real-LLM e2es (a model designs a trigger rule; a model chooses facts to remember). triggers 74/74, memory 42/42 green.
- **live-agents collapsed — Phase 4 SQL de-duplication COMPLETE across the framework.** `@weaveintel/live-agents`' StateStore (`weavePostgresStateStore` / `weaveSqliteStateStore`) had its ~300 lines of Proxy + entity-dispatch + 21-case hydration boilerplate copy-pasted into both adapters, differing only in two tiny SQL queries. Now that whole machine lives once in `drizzle-state-store.ts` — the two queries are Drizzle, the sync/async difference is the shared `exec` seam, and the 21-case hydration switch collapses to a single data-driven loop. Same public factory APIs (incl. Phase 2 `{url}|{pool}`), on-disk `la_entities` shape unchanged (backward compatible). Proven on real Postgres (Testcontainers) — the pg adapter was never run against a database in-package before: durability/crash-recovery (state written by one store rehydrates in a fresh one), `claimNextTicks` array-mutation persistence, a 1,000-agent rehydration stress, injection safety, and a real-LLM roster-survives-restart e2e. live-agents 89/89 + 5/5 real-pg green.
- **Every SQL-backed framework store is now behind a single Drizzle implementation per store:** workflows (10 stores), triggers, memory, live-agents. Only non-SQL backends (Mongo/Redis/DynamoDB) and pgvector remain dialect-specific by nature. **Phase 4 done.** (Phase 5 — cutover/retire — is an app-level rollout, not a library task.)
