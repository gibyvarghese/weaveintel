...
export interface GdriveCredentials { accessToken: string; }
...
function extractCredentials(ctx: ExecutionContext): GdriveCredentials {
  const token = ctx.metadata?.['gdriveAccessToken'] as string | undefined;
...
export const liveGdriveAdapter: GdriveAdapter = {
  async listFiles(creds, folderId, pageSize) {
    ...
    const data = await driveFetch(creds.accessToken, `${DRIVE_BASE}/files?q=${q}&pageSize=${pageSize}&fields=files(id,name,mimeType,size,modifiedTime,parents,webViewLink)`) as Record<string, unknown>;
    return ((data['files'] as Array<Record<string, unknown>>) ?? []).map(parseFile);
  },
  async readFile(creds, fileId) {
    const meta = await driveFetch(creds.accessToken, `${DRIVE_BASE}/files/${fileId}?fields=id,name,mimeType,size,modifiedTime,parents,webViewLink`) as Record<string, unknown>;
    const content = await driveFetch(creds.accessToken, `${DRIVE_BASE}/files/${fileId}?alt=media`) as string;
  ...
  async createFile(creds, name, content, mimeType, parentId) {
    ...
    const raw = await driveFetch(creds.accessToken, `${UPLOAD_BASE}/files?uploadType=multipart`, { method: 'POST', body: form }) as Record<string, unknown>;
    return parseFile(raw);
  },
  async updateFile(creds, fileId, content) {
    const raw = await driveFetch(creds.accessToken, `${UPLOAD_BASE}/files/${fileId}?uploadType=media`, { method: 'PATCH', headers: { 'Content-Type': 'text/plain' }, body: content }) as Record<string, unknown>;
    return parseFile(raw);
  },
  async shareFile(creds, fileId, email, role) {
    await driveFetch(creds.accessToken, `${DRIVE_BASE}/files/${fileId}/permissions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'user', role, emailAddress: email }) });
  },
  async subscribeChanges(_creds, _onFile) { return { stop: () => {} }; },
};

export interface GdriveMCPServerOptions { adapter?: GdriveAdapter; }
