...
/**
 * @weaveintel/coedit — Agent as a co-editing peer (Collaboration Phase 7).
 *
 ...
 */
import type { RgaDoc, RgaOp } from './rga.js';
...
export interface AgentPeer {
  ...
  append(text: string): RgaOp[];
  /** Insert text at a visible index; returns the ops. */
  insertAt(index: number, text: string): RgaOp[];
...
/**
 * Wrap an {@link RgaDoc} (whose `siteId` should be an {@link agentSiteId}) as an
 ...
 */
export function createAgentPeer(doc: RgaDoc, opts: AgentPeerOptions = {}): AgentPeer {
  const mode = opts.mode ?? 'direct';
  let written = 0;

  const emit = (index: number, text: string): RgaOp[] => {
    if (text.length === 0) return [];
    if (mode === 'suggest') {
      // Compute ops against a private clone so the live doc is untouched.
      const clone = doc.constructor as typeof RgaDoc;
      const shadow = clone.fromSnapshot(doc.siteId, doc.snapshot());
      const ops = shadow.localInsertText(index, text);
      written += [...text].length;
      return ops;
    }
