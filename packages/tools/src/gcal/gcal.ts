...
export interface GcalCredentials { accessToken: string; calendarId?: string; refreshAccessToken?: () => Promise<string | undefined>; }
export interface GcalTokenProvider {
...
function createMetadataTokenProvider(): GcalTokenProvider {
  return {
    async getToken(ctx) {
      return ctx.metadata?.['gcalAccessToken'] as string | undefined;
    },
    async refreshToken(ctx) {
      return ctx.metadata?.['gcalRefreshAccessToken'] as string | undefined;
    },
  };
}

async function extractCredentials(ctx: ExecutionContext, provider: GcalTokenProvider): Promise<GcalCredentials> {
...
async function calFetch(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const resp = await hardenedFetch(url, { ...init, headers: { Authorization: `Bearer ${activeToken}`, 'Content-Type': 'application/json', ...(init?.headers as Record<string, string> | undefined) } }, { errorTag: 'tools-gcal' });
  ...
  throw new Error('Calendar API request failed after token refresh attempt');
}
...
export const liveGcalAdapter: GcalAdapter = {
  async listEvents(creds, timeMin, timeMax, maxResults) {
    ...
    const data = await calFetch(creds.accessToken, `${GCAL_BASE}/calendars/${calId}/events?timeMin=${timeMin}&timeMax=${timeMax}&maxResults=${maxResults}&singleEvents=true&orderBy=startTime`, undefined, creds.refreshAccessToken) as Record<string, unknown>;
  ...
  async createEvent(creds, event) {
    ...
    const raw = await calFetch(creds.accessToken, `${GCAL_BASE}/calendars/${calId}/events`, { method: 'POST', body: JSON.stringify(body) }, creds.refreshAccessToken) as Record<string, unknown>;
    return parseEvent(raw);
  },
  async updateEvent(creds, eventId, patch) {
    const calId = encodeURIComponent(creds.calendarId ?? 'primary');
    const raw = await calFetch(creds.accessToken, `${GCAL_BASE}/calendars/${calId}/events/${encodeURIComponent(eventId)}`, { method: 'PATCH', body: JSON.stringify(patch) }, creds.refreshAccessToken) as Record<string, unknown>;
    return parseEvent(raw);
  },
  async rsvp(creds, eventId, email, response) {
    const calId = encodeURIComponent(creds.calendarId ?? 'primary');
    const raw = await calFetch(creds.accessToken, `${GCAL_BASE}/calendars/${calId}/events/${encodeURIComponent(eventId)}?sendUpdates=none`, { method: 'PATCH', body: JSON.stringify({ attendees: [{ email, responseStatus: response }] }) }, creds.refreshAccessToken) as Record<string, unknown>;
    return parseEvent(raw);
  },
  async subscribeChanges() { return { stop: () => {} }; },
};

export interface GcalMCPServerOptions { adapter?: GcalAdapter; tokenProvider?: GcalTokenProvider; }
