...
| `nextStreamAnnouncement()` / `computeAppendedText()` | **Streaming announcements** (stream-announce): the pure, accessible live-region POLICY for a streaming AI answer. Naive streaming re-renders the transcript every token, so a screen reader re-reads the whole conversation each token — unusable. This decides WHAT a visually-hidden `role="status"` region should say and WHEN: "Generating response…" then the whole answer once (`summary`, the quiet default), sentence-complete chunks on a throttle (`live`), or nothing (`off`). No DOM/timers — the app owns the region; geneWeave mirrors this in the raw-served UI (which can't bare-import a workspace package) with these tests as the spec. |
| `makeVariantStack()` / `addVariant()` / `selectVariant()` / `variantLabel()` | **Answer variants** (answer-variants): the pure "stack of regenerated answers with an active pointer" model behind Regenerate-with-version-history. `addVariant` is append-only (the new answer becomes active) and prunes only the OLDEST beyond `maxKept` (never the one on screen); `selectVariant` switches losslessly (clamps out-of-range, never throws); `variantLabel` produces the "2/3" pager + prev/next affordances. No I/O — geneWeave persists the variants in `message_variants` and mirrors the active one into the live `messages` row, so the normal transcript is unchanged. |
| `validateMessageFeedback()` / `summarizeMessageFeedback()` | **Answer feedback** (message-feedback): the pure taxonomy + validation + aggregation behind a thumbs up/down on a single chat answer. A down-vote may carry a small **fixed reason taxonomy** (`FEEDBACK_CATEGORIES`: Not accurate / Incomplete / Didn't follow instructions / Unsafe / …) so a complaint is actionable, not just a number — the same idea as a run annotation, on the chat surface. `sanitizeFeedbackCategories()` keeps only known reason keys (drops junk / injection); `signalToRating()` maps the platform's `thumbs_up`/`thumbs_down`/`copy`/`regenerate` signal vocabulary onto up/down for satisfaction stats. geneWeave persists these on its existing `message_feedback` store and feeds the routing quality loop. |
| `selectSuggestions()` / `buildSuggestPromptsPrompt()` / `parseSuggestedPromptsReply()` | **Suggested / starter prompts** (suggested-prompts): the pure engine behind the empty chat's clickable "conversation starters". `selectSuggestions()` combines a small **curated** default set (`CURATED_PROMPTS`, varied to advertise the core capabilities) with **personalised** candidates built from the reader's own recent notes/chats (`buildNoteCandidates` / `buildChatCandidates`), then dedupes, ranks (personalised first) and caps them. Titles are untrusted, so `sanitizePromptText()` strips control chars / collapses whitespace / clamps length (no line-injection, no UI break). For AI-personalised starters, `buildSuggestPromptsPrompt()` spotlights the titles as DATA and `parseSuggestedPromptsReply()` safely parses the model's JSON (tolerates fences/prose, drops malformed, coerces categories). geneWeave persists a per-tenant policy + a per-user cache + a click log, and exposes it via `suggest_prompts` + the empty-chat cards. |
