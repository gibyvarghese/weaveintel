/**
 * Hardened fetch for news adapters. Default-on — no opt-in flag.
 *
 * Mitigates:
...
function assertHttpsOrLoopback(url: string): void {
  let parsed: URL;
  try { parsed = new URL(url); } catch { throw new Error(`tools-news: invalid URL "${url}"`); }
  if (parsed.protocol === 'https:') return;
  if (parsed.protocol === 'http:' && isLoopback(parsed.hostname)) return;
  throw new Error(`tools-news: refusing non-HTTPS request to "${parsed.hostname}" (only loopback may use http://).`);
}
  ...
  return new ReadableStream<Uint8Array>({
    async pull(controller) {
      try {
        ...
        if (total > maxBytes) {
          await reader.cancel();
          controller.error(new Error(`tools-news: response body exceeded ${maxBytes} bytes`));
          return;
        }
...
export async function newsFetch(
  ...
  if (cl) {
    ...
    if (Number.isFinite(n) && n > maxBytes) {
      ...
      throw new Error(`tools-news: response Content-Length ${n} exceeds limit ${maxBytes}`);
    }
  }
