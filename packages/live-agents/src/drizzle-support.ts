...
/**
 * The one dialect seam for the Drizzle-backed state store. Drizzle's query builder reads the same
...
/** A strictly-increasing ISO clock so `updated_at` never ties → stable hydration order on any database. */
