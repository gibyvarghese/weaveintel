...
| **6** | **M17 — production hardening: capability-matrix in-memory cache (60s TTL), `safeTranslate` circuit breaker, A/B routing experiments (`routing_experiments` table + `pickExperimentCandidate`), cost telemetry by `task_key` (`aggregateCostByTask` + admin `/cost-by-task`)** | ✅ shipped (see [`examples/74-routing-phase6-production.ts`](../../examples/74-routing-phase6-production.ts)) |
