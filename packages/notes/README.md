...
| `sm2` / `initialSchedule` / `dueCards` / `studyStats` / `validateFlashcards` | Phase 5 STUDY model: flashcards + the **SM-2** spaced-repetition scheduler. `sm2(schedule, rating, now)` is the ~30-line classic SuperMemo-2 maths (ease factor, interval ladder 1→6→×ease, reset on a fail); `dueCards`/`studyStats` drive the review queue + deck stats; `validateFlashcards` is the strict gate over AI-/user-supplied cards. (FSRS is a more accurate trained successor; the `CardSchedule` shape is forward-compatible.) Pure + injectable time → fully testable. |
