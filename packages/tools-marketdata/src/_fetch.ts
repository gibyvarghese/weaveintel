/**
 * Hardened fetch for marketdata adapters. Default-on — no opt-in flag.
 *
 * Mitigates:
 *   - hung vendor requests → request-level timeout via AbortSignal.
...
function assertHttpsOrLoopback(url: string): void {
  let parsed: URL;
  try { parsed = new URL(url); } catch { throw new Error(`tools-marketdata: invalid URL "${url}"`); }
  if (parsed.protocol === 'https:') return;
  if (parsed.protocol === 'http:' && isLoopback(parsed.hostname)) return;
  throw new Error(`tools-marketdata: refusing non-HTTPS request to "${parsed.hostname}" (only loopback may use http://).`);
}
  ...
  return new ReadableStream<Uint8Array>({
    async pull(controller) {
      try {
        ...
        if (total > maxBytes) {
          await reader.cancel();
          controller.error(new Error(`tools-marketdata: response body exceeded ${maxBytes} bytes`));
          return;
        }
...
export async function marketdataFetch(
  ...
  if (cl) {
    ...
    if (Number.isFinite(n) && n > maxBytes) {
      ...
      throw new Error(`tools-marketdata: response Content-Length ${n} exceeds limit ${maxBytes}`);
    }
  }
