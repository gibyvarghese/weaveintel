...
export interface SlackCredentials { botToken: string; refreshBotToken?: () => Promise<string | undefined>; }
...
function createMetadataTokenProvider(): SlackTokenProvider {
  return {
    async getToken(ctx) {
      return ctx.metadata?.['slackBotToken'] as string | undefined;
    },
    async refreshToken(ctx) {
      return ctx.metadata?.['slackRefreshBotToken'] as string | undefined;
    },
  };
}

async function extractCredentials(ctx: ExecutionContext, provider: SlackTokenProvider): Promise<SlackCredentials> {
...
async function slackFetch(
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (!data['ok'] && data['error'] === 'invalid_auth' && attempt === 0 && refreshToken) {
      const refreshed = await refreshToken();
      if (refreshed) {
        activeToken = refreshed;
        continue;
  ...
  throw new Error('Slack request failed after token refresh attempt');
}
...
export const liveSlackAdapter: SlackAdapter = {
  async postMessage(creds, channel, text, threadTs) {
    ...
    const data = await slackFetch(creds.botToken, 'chat.postMessage', params, creds.refreshBotToken);
  ...
  async readChannel(creds, channel, limit, cursor) {
    ...
    const data = await slackFetch(creds.botToken, 'conversations.history', params, creds.refreshBotToken);
  ...
  async search(creds, query, count) {
    const data = await slackFetch(creds.botToken, 'search.messages', { query, count }, creds.refreshBotToken);
...
export interface SlackMCPServerOptions { adapter?: SlackAdapter; tokenProvider?: SlackTokenProvider; }
...
export function createSlackMCPServer(opts: SlackMCPServerOptions = {}) {
  ...
  server.addTool({ name: 'slack.post', description: 'Post a message to a Slack channel or thread.', inputSchema: { type: 'object', properties: { channel: { type: 'string' }, text: { type: 'string' }, threadTs: { type: 'string' } }, required: ['channel', 'text'] } }, async (ctx, args) => {
    ...
    creds.refreshBotToken = async () => tokenProvider.refreshToken(ctx);
