...
| `peerColor` / `AI_PARTICIPANT` / `aiPeerId` / `isAiPeerId` / `sanitizeAwarenessState` | **Live-presence helpers** (weaveNotes Phase 3): a stable, accessible per-peer cursor colour (excluding the AI-reserved emerald), the synthetic "weaveIntel AI" participant identity the server announces while the agent edits, and a strict sanitiser the server runs over an incoming awareness frame (cap names, validate the colour, bound the cursor, drop unknown keys) — presence is un-authenticated chatter, so it must be bounded + inert. |
