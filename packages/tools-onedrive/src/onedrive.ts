...
export interface OnedriveCredentials { accessToken: string; userId?: string; refreshAccessToken?: () => Promise<string | undefined>; }
export interface OnedriveTokenProvider {
...
function createMetadataTokenProvider(): OnedriveTokenProvider {
  return {
    async getToken(ctx) {
      return ctx.metadata?.['onedriveAccessToken'] as string | undefined;
    },
    async refreshToken(ctx) {
      return ctx.metadata?.['onedriveRefreshAccessToken'] as string | undefined;
    },
  };
}

async function extractCredentials(ctx: ExecutionContext, provider: OnedriveTokenProvider): Promise<OnedriveCredentials> {
...
async function driveFetch(
  ...
  throw new Error('Graph OneDrive request failed after token refresh attempt');
}
...
export const liveOnedriveAdapter: OnedriveAdapter = {
  async listItems(creds, path) {
    ...
    const data = await driveFetch(creds.accessToken, endpoint, undefined, creds.refreshAccessToken) as Record<string, unknown>;
    return ((data['value'] as Array<Record<string, unknown>>) ?? []).map(parseItem);
  },
  async readItem(creds, itemId) {
    const base = driveBase(creds.userId);
    const meta = await driveFetch(creds.accessToken, `${base}/items/${itemId}`, undefined, creds.refreshAccessToken) as Record<string, unknown>;
    const content = await driveFetch(creds.accessToken, `${base}/items/${itemId}/content`, undefined, creds.refreshAccessToken) as string;
    return { item: parseItem(meta), content: String(content) };
  },
  async createItem(creds, path, content) {
    const base = driveBase(creds.userId);
    const raw = await driveFetch(creds.accessToken, `${base}/root:${path}:/content`, { method: 'PUT', headers: { 'Content-Type': 'text/plain' }, body: content }, creds.refreshAccessToken) as Record<string, unknown>;
    return parseItem(raw);
  },
  async updateItem(creds, itemId, content) {
    const base = driveBase(creds.userId);
    const raw = await driveFetch(creds.accessToken, `${base}/items/${itemId}/content`, { method: 'PUT', headers: { 'Content-Type': 'text/plain' }, body: content }, creds.refreshAccessToken) as Record<string, unknown>;
    return parseItem(raw);
  },
  async shareItem(creds, itemId, email, role) {
    const base = driveBase(creds.userId);
    const raw = await driveFetch(creds.accessToken, `${base}/items/${itemId}/invite`, { method: 'POST', body: JSON.stringify({ recipients: [{ email }], roles: [role], sendInvitation: false }) }, creds.refreshAccessToken) as Record<string, unknown>;
...
export interface OnedriveMCPServerOptions { adapter?: OnedriveAdapter; tokenProvider?: OnedriveTokenProvider; }
