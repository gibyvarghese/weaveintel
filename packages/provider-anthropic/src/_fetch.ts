 ...
 * file is the same shape with `provider-anthropic` namespacing on errors.
 *
 * Anthropic SSE streams (Messages streaming, batch JSONL results) call
...
export function assertHttpsOrLoopback(url: string): void {
  ...
  try {
    ...
    throw new Error(`provider-anthropic: invalid URL "${url}"`);
  }
  if (parsed.protocol === 'https:') return;
  if (parsed.protocol === 'http:' && isLoopback(parsed.hostname)) return;
  throw new Error(
    `provider-anthropic: refusing non-HTTPS request to "${parsed.hostname}" (only loopback may use http://).`,
  );
}
  ...
  return new ReadableStream<Uint8Array>({
    async pull(controller) {
      try {
        ...
        if (total > maxBytes) {
          await reader.cancel();
          controller.error(new Error(`provider-anthropic: response body exceeded ${maxBytes} bytes`));
          return;
        }
...
export async function anthropicFetch(
  ...
  if (cl) {
    ...
    if (Number.isFinite(n) && n > maxBytes) {
      ...
      throw new Error(`provider-anthropic: response Content-Length ${n} exceeds limit ${maxBytes}`);
    }
  }
