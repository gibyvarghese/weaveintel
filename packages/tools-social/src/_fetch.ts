/**
 * Hardened fetch for social platform adapters. Default-on — no opt-in flag.
 *
 * Mitigates:
 *   - hung platform requests → request-level timeout via AbortSignal.
...
function assertHttpsOrLoopback(url: string): void {
  let parsed: URL;
  try { parsed = new URL(url); } catch { throw new Error(`tools-social: invalid URL "${url}"`); }
  if (parsed.protocol === 'https:') return;
  if (parsed.protocol === 'http:' && isLoopback(parsed.hostname)) return;
  throw new Error(`tools-social: refusing non-HTTPS request to "${parsed.hostname}" (only loopback may use http://).`);
}
  ...
  return new ReadableStream<Uint8Array>({
    async pull(controller) {
      try {
        ...
        if (total > maxBytes) {
          await reader.cancel();
          controller.error(new Error(`tools-social: response body exceeded ${maxBytes} bytes`));
          return;
        }
...
export async function socialFetch(
  ...
  if (cl) {
    ...
    if (Number.isFinite(n) && n > maxBytes) {
      ...
      throw new Error(`tools-social: response Content-Length ${n} exceeds limit ${maxBytes}`);
    }
  }
