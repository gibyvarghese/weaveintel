...
describeAdmin('Admin SGAP', () => {
  ...
  it('creates an SG brand', async () => {
    const hasAuth = await ensureAuthenticated();
    const { status, data } = await api('POST', '/api/admin/sg-brands', {
      name: 'Test Growth Brand',
      slug: `test-growth-${Date.now()}`,
      description: 'Admin SGAP integration test brand',
      voice: 'Direct and practical',
      goals_json: { north_star: 'qualified_leads' },
      enabled: true,
    });
    if (!hasAuth) { expect(status).toBe(401); return; }
    if (status === 403) { expect(typeof data['error']).toBe('string'); return; }
    expect(status).toBe(201);
    const row = data['sg-brand'] as Record<string, unknown>;
    expect(row?.['id']).toBeDefined();
    brandId = row['id'] as string;
  });

  it('creates SG channel and campaign for the brand', async () => {
    const hasAuth = await ensureAuthenticated();
    const ch = await api('POST', '/api/admin/sg-channels', {
      brand_id: brandId,
      platform: 'linkedin',
      handle: '@testbrand',
      status: 'active',
    ...
    if (!hasAuth) { expect(ch.status).toBe(401); return; }
    if (ch.status === 403) { expect(typeof ch.data['error']).toBe('string'); return; }
    expect(ch.status).toBe(201);
    channelId = ((ch.data['sg-channel'] as Record<string, unknown>)?.['id'] as string) ?? '';
    expect(channelId.length).toBeGreaterThan(0);

    const cp = await api('POST', '/api/admin/sg-campaigns', {
      brand_id: brandId,
    ...
    if (cp.status === 403) { expect(typeof cp.data['error']).toBe('string'); return; }
    expect(cp.status).toBe(201);
    campaignId = ((cp.data['sg-campaign'] as Record<string, unknown>)?.['id'] as string) ?? '';
    expect(campaignId.length).toBeGreaterThan(0);
  });

  it('creates strategy setting and content queue item', async () => {
    const hasAuth = await ensureAuthenticated();
    const st = await api('POST', '/api/admin/sg-strategy-settings', {
      brand_id: brandId,
      key: 'content_mix',
      value_json: { educational: 0.6, advisory: 0.4 },
      enabled: true,
    });
    if (!hasAuth) { expect(st.status).toBe(401); return; }
    if (st.status === 403) { expect(typeof st.data['error']).toBe('string'); return; }
    expect(st.status).toBe(201);
    strategyId = ((st.data['sg-strategy-setting'] as Record<string, unknown>)?.['id'] as string) ?? '';

    const cq = await api('POST', '/api/admin/sg-content-queue', {
      brand_id: brandId,
      campaign_id: campaignId,
      channel_id: channelId,
      title: 'How to pick your first AI eval stack',
      brief: 'Short practical checklist for startup teams.',
      format: 'article',
      status: 'draft',
      enabled: true,
    });
    if (cq.status === 403) { expect(typeof cq.data['error']).toBe('string'); return; }
    expect(cq.status).toBe(201);
    contentId = ((cq.data['sg-content-item'] as Record<string, unknown>)?.['id'] as string) ?? '';
    expect(contentId.length).toBeGreaterThan(0);
  });

  it('creates and runs an SG workflow template', async () => {
    const hasAuth = await ensureAuthenticated();
    const tmpl = await api('POST', '/api/admin/sg-workflow-templates', {
      brand_id: brandId,
      name: 'Integration Test SG Workflow',
      description: 'Test template',
      step_graph_json: { steps: [{ id: 'measure', type: 'measure' }] },
      trigger_type: 'manual',
      enabled: true,
    });
    if (!hasAuth) { expect(tmpl.status).toBe(401); return; }
    if (tmpl.status === 403) { expect(typeof tmpl.data['error']).toBe('string'); return; }
    expect(tmpl.status).toBe(201);
    workflowTemplateId = ((tmpl.data['sg-workflow-template'] as Record<string, unknown>)?.['id'] as string) ?? '';
    expect(workflowTemplateId.length).toBeGreaterThan(0);

    const run = await api('POST', `/api/admin/sg-workflow-templates/${workflowTemplateId}/run`, {
      brand_id: brandId,
      channel_id: channelId,
    });
    if (run.status === 403) { expect(typeof run.data['error']).toBe('string'); return; }
    expect(run.status).toBe(201);
    expect((run.data['run'] as Record<string, unknown>)?.['status']).toBe('completed');
    expect((run.data['snapshot'] as Record<string, unknown>)?.['brand_id']).toBe(brandId);
  });

  it('lists and updates SG entities', async () => {
    const hasAuth = await ensureAuthenticated();
    const list = await api('GET', '/api/admin/sg-brands');
    if (!hasAuth) { expect(list.status).toBe(401); return; }
    if (list.status === 403) { expect(typeof list.data['error']).toBe('string'); return; }
    expect(list.status).toBe(200);
    expect(Array.isArray(list.data['sg-brands'])).toBe(true);

    const upd = await api('PUT', `/api/admin/sg-content-queue/${contentId}`, {
      status: 'ready',
      metadata_json: { cta: 'newsletter_signup' },
    });
    if (upd.status === 403) { expect(typeof upd.data['error']).toBe('string'); return; }
    expect(upd.status).toBe(200);
    expect((upd.data['sg-content-item'] as Record<string, unknown>)?.['status']).toBe('ready');
  });

  it('deletes SG entities', async () => {
    const hasAuth = await ensureAuthenticated();
    const delTemplate = await api('DELETE', `/api/admin/sg-workflow-templates/${workflowTemplateId}`);
    if (!hasAuth) { expect(delTemplate.status).toBe(401); return; }
    if (delTemplate.status === 403) { expect(typeof delTemplate.data['error']).toBe('string'); return; }
    expect(delTemplate.status).toBe(200);
    expect(delTemplate.data['ok']).toBe(true);

    const delContent = await api('DELETE', `/api/admin/sg-content-queue/${contentId}`);
    if (delContent.status === 403) { expect(typeof delContent.data['error']).toBe('string'); return; }
    expect(delContent.status).toBe(200);
    expect(delContent.data['ok']).toBe(true);

    const delStrategy = await api('DELETE', `/api/admin/sg-strategy-settings/${strategyId}`);
    if (delStrategy.status === 403) { expect(typeof delStrategy.data['error']).toBe('string'); return; }
    expect(delStrategy.status).toBe(200);
    expect(delStrategy.data['ok']).toBe(true);

    const delCampaign = await api('DELETE', `/api/admin/sg-campaigns/${campaignId}`);
    if (delCampaign.status === 403) { expect(typeof delCampaign.data['error']).toBe('string'); return; }
    expect(delCampaign.status).toBe(200);
    expect(delCampaign.data['ok']).toBe(true);

    const delChannel = await api('DELETE', `/api/admin/sg-channels/${channelId}`);
    if (delChannel.status === 403) { expect(typeof delChannel.data['error']).toBe('string'); return; }
    expect(delChannel.status).toBe(200);
    expect(delChannel.data['ok']).toBe(true);

    const delBrand = await api('DELETE', `/api/admin/sg-brands/${brandId}`);
    if (delBrand.status === 403) { expect(typeof delBrand.data['error']).toBe('string'); return; }
    expect(delBrand.status).toBe(200);
    expect(delBrand.data['ok']).toBe(true);
  });
});
