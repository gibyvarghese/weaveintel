...
export const PLATFORM_CAPABILITY_ADMIN_TABS: Record<string, AdminTabDef> = {
  ...
  'memory-governance': {
    ...
    cols: ['name', 'max_age', 'max_entries', 'enabled'],
    fields: [
      { key: 'name', label: 'Name' },
      { key: 'description', label: 'Description', textarea: true, rows: 2 },
      {
        key: 'memory_types', label: 'Memory Types (JSON — leave blank for all)', save: 'json',
        options: ['semantic', 'user_fact', 'preference', 'summary'],
        textarea: true, rows: 2,
      },
      { key: 'tenant_id', label: 'Tenant ID (optional — blank = global)' },
      { key: 'block_patterns', label: 'Block Patterns (JSON regex array)', textarea: true, rows: 3, save: 'json' },
      { key: 'redact_patterns', label: 'Redact Patterns (JSON regex array)', textarea: true, rows: 3, save: 'json' },
      {
        key: 'max_age', label: 'Max Age (ISO 8601 duration)',
        options: ['', 'P7D', 'P30D', 'P90D', 'P180D', 'P1Y'],
        default: '',
      },
      { key: 'max_entries', label: 'Max Entries per User', type: 'number', save: 'int' },
      { key: 'enabled', label: 'Enabled', type: 'checkbox', save: 'bool', default: true },
    ],
  },
