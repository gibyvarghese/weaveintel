...
export const PLATFORM_CAPABILITY_ADMIN_TABS: Record<string, AdminTabDef> = {
  ...
  'capability-policy-bindings': {
    ...
    fields: [
      { key: 'binding_kind', label: 'Binding Kind (agent | mesh | workflow)' },
      { key: 'binding_ref', label: 'Binding Ref (id matching binding_kind)' },
      { key: 'policy_kind', label: 'Policy Kind (tool_policy | guardrail | routing | memory | sandbox)' },
      { key: 'policy_ref', label: 'Policy Ref (id of the policy row)' },
      { key: 'precedence', label: 'Precedence (agent=100 / mesh=50 / workflow=10)', type: 'number', save: 'int', default: 10 },
      { key: 'enabled', label: 'Enabled', type: 'checkbox', save: 'bool', default: true },
    ],
  },
  'capability-packs': {
