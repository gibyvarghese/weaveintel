...
/**
 * M-6: Extended weak-secret detection patterns. The original list only caught
 * simple placeholder words; it missed:
 *  - Repeated characters (aaaa…, 1111…) — zero entropy.
 *  - Keyboard walks (qwerty, asdf, 1234…) — trivially guessable.
 *  - Canonical "change me" placeholders like CHANGE_ME, todo, fixme.
 *  - Padding tricks (secret123…) — common CI template pattern.
 *  - All-zeros/all-ones hex strings — sometimes generated by broken tooling.
 *  - UUID v4 format — structurally predictable; 122 random bits is adequate but
 *    UUIDs generated by weak RNGs (Math.random) may have far less entropy.
 *
 * Length check is intentionally kept: even a random-looking 16-char string
 * falls below the 256-bit bar expected for HS-256 JWT secrets.
 */
const WEAK_PATTERNS: RegExp[] = [
  // Exact-match placeholder words
  /^(secret|password|changeme|change[-_]?me|todo|fixme|example|test|dev|placeholder|your[-_]?secret|insert[-_]?secret)$/i,
  // Padding tricks: word followed by digits only
  /^(secret|password|changeme)[0-9]+$/i,
  // Repeated single character (e.g. aaaa…, 1111…)
  /^(.)\1{15,}$/,
  // Keyboard walk patterns
  /^(qwerty|asdfgh|zxcvbn|abcdef|123456|654321|abcdefghij|1234567890)/i,
  // All-zeros / all-ones hex (32 chars = 128 bits but entropy = 0)
  /^0{32,}$/,
  /^f{32,}$/i,
];
