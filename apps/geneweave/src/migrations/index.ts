...
import { applyM110NoteFlashcards } from './m110-note-flashcards.js';
...
import { applyM133VoiceMeeting } from './m133-voice-meeting.js';
import { applyM134BackgroundMemory } from './m134-background-memory.js';
import { applyM135TenantAppearance } from './m135-tenant-appearance.js';
...
const bootstrapRunner = createMigrationRunner([
  ...
  { id: 'm110-note-flashcards', description: 'weaveNotes Phase 5: AI study — adds note_flashcards (one row per card: front/back + the SM-2 schedule ease_factor/interval_days/repetitions/due_at/last_reviewed_at; owner-scoped, tenant-isolated, cascades on note delete, indexed by note + by (owner, due_at)) + weavenotes_settings.flashcards_enabled + daily_new_card_limit. Registers the make_flashcards tool in tool_catalog, grants it to the weaveNotes Editor agent, and enables it. "Make flashcards from this note" produces a reviewable deck scheduled with spaced repetition. Idempotent', run: applyM110NoteFlashcards },
  ...
  { id: 'm133-voice-meeting', description: 'weaveNotes Phase 4: voice / meeting capture. Adds note_meetings (one row per recording: timestamped transcript segments, structured summary/decisions/action-items, citation-coverage counts, duration, source, audio-retained flag; owner-scoped, tenant-isolated) + weavenotes_settings dials (voice_capture_enabled on; store_audio OFF for privacy; transcription_language auto; transcription_model whisper-1; max_recording_seconds 3600) + the summarize_meeting tool in tool_catalog granted to the weaveNotes Editor agent. Record → transcribe → structured note where every point links to the moment it was said. Idempotent', run: applyM133VoiceMeeting },
  { id: 'm134-background-memory', description: 'weaveNotes Phase 5: background memory ("second brain"). Adds note_memory_state (per-note content hash + produced memory ids, so unchanged notes are not re-processed) + weavenotes_settings dials (background_memory_enabled on; memory_importance_threshold 0.3; memory_max_per_note 8; memory_recall_count 5; memory_decay_half_life_days 30) + the recall_second_brain tool granted to the weaveNotes Editor + a new weavenotes_memory worker agent. A background job distils durable facts/preferences/decisions from notes into the shared @weaveintel/memory store; recall is temporally aware (recent+important+relevant first, superseded excluded). Idempotent', run: applyM134BackgroundMemory },
  { id: 'm135-tenant-appearance', description: 'geneWeave UI rebuild: per-tenant Appearance / branding (white-label). Adds tenant_appearance (one row per tenant: brand name, logo, brand accent, corner style, fonts, default colour-scheme system/light/dark, Pro/Creative default) applied as CSS custom properties at runtime and accessibility-enforced by @geneweave/tokens (a brand colour failing WCAG-AA on a theme falls back to the accessible default). Plus the set_workspace_appearance tool granted to the weaveNotes Editor. Idempotent', run: applyM135TenantAppearance },
]);
